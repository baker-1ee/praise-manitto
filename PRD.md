...
```
입력: 팀원 N명 + 최근 스프린트 이력(가중치)
조건: 팀 전체가 하나의 순환(Hamiltonian cycle)을 이루도록 강제
      → 자기 자신 배정 불가(derangement) 자동 만족
      → A→B, B→A 동시 발생(2인 상호 배정) 구조적으로 불가능 (N=2 팀 제외 — 인원이 둘뿐이라 불가피)
가중치: 최근 M(N) = clamp(⌈(N-1)/2⌉, 3, 20)개 스프린트의 manito↔target 조합에
        지수 감쇠(half-life 2스프린트) 가중치 부여 — 최근에 짝지어졌을수록 비용이 높아
        다시 짝지어질 확률이 낮아짐 (하드 컷오프 없음, 배정 불가 상황 없음)
        탈퇴자 이력은 자동 무시, 신규 합류자는 가중치 0(최우선 배정 후보)
방법: 가중치 합이 최소인 순환 탐색
      N ≤ 8  → 전수 탐색 (N-1)! 완전 최적해
      N > 8  → 무작위 시작점 최대 10개 그리디 최근접 이웃 + 2-opt 로컬서치, 최저 비용 채택
